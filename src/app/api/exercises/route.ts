...
export function GET(request: Request) {
  ...
  const matches = filtered.slice(offset, offset + limit).map((exercise) => ({ id: exercise.id, name: exercise.n, bodyPart: exercise.bp, target: exercise.tg, equipment: exercise.eq, image: exercise.img, gif: exercise.gif, steps: exercise.st }));
