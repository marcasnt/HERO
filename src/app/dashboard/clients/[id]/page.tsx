...
export default async function ClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const bodyFat = measures.slice().reverse().map((measure) => Number((measure.values as { bodyFat?: number }).bodyFat)).filter((value) => Number.isFinite(value) && value > 0);
  ...
  return <main className="dashboard"><div className="page-title"><div><span className="kicker">EXPEDIENTE DEL CLIENTE</span><h1>{client.name}</h1><p>{client.email}</p></div><Link className="button" href="/dashboard/routines">Asignar rutina</Link></div>
    ...
    <section className="analytics-grid"><MiniChart values={weights} label="Peso corporal" suffix=" kg"/><MiniChart values={bodyFat} label="Grasa estimada" suffix="%"/><MiniChart values={volumes} label="Volumen por sesión" suffix=" kg"/><div className="chart metric"><div><span>Adherencia registrada</span><strong>{assignments.length ? Math.min(100, Math.round((sessions.length / Math.max(assignments.length * 4, 1)) * 100)) : 0}%</strong></div><p>{sessions.length} sesiones completadas</p></div></section>
    <section className="dashboard-grid"><div className="panel"><div className="panel-head"><h2>Actividad reciente</h2></div>{sessions.length ? <div className="timeline">{sessions.map((session) => <div key={session.id}><span className="dot"/><span><b>Rutina completada</b><small>{session.completedAt?.toLocaleDateString("es")}{session.clientFeedback ? ` · ${session.clientFeedback}` : ""}</small></span></div>)}</div> : <div className="empty">Sin sesiones registradas.</div>}</div>
      <div className="panel"><div className="panel-head"><h2>Mediciones</h2></div>{measures.length ? <div className="timeline">{measures.map((measure) => { const values = measure.values as { weight?: number; waist?: number; waistCm?: number; bodyFat?: number; neckCm?: number; hipCm?: number }; return <div key={measure.id}><span className="dot"/><span><b>{values.weight || "—"} kg · {values.bodyFat ? `${values.bodyFat}% grasa est.` : "sin estimación"}</b><small>Cintura {values.waistCm || values.waist || "—"} cm · Cuello {values.neckCm || "—"} cm · Cadera {values.hipCm || "—"} cm · {measure.measuredAt.toLocaleDateString("es")}</small></span></div>; })}</div> : <div className="empty">Sin mediciones.</div>}</div>
    </section>
