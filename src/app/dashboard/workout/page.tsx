...
export default async function WorkoutPage() {
  ...
  return <main className="dashboard workout-screen"><div className="page-title"><div><span className="kicker">SEGUIR</span><h1>Entrenamiento</h1><p>Registra peso, repeticiones y esfuerzo serie por serie.</p></div></div>{assignments.length ? assignments.map((assignment) => { const routine = assignment.definition as Routine; const all = routine.exercises || []; const todayExercises = all.filter((exercise) => (exercise.day || "Lunes") === today); const exercises = todayExercises.length ? todayExercises : all; return <section className="panel active-workout" key={assignment.id}><div className="workout-heading"><div><span className="kicker">{todayExercises.length ? "RUTINA DE HOY" : "RUTINA DISPONIBLE"}</span><h2>{routine.dayNames?.[today] || assignment.name}</h2></div><b>{exercises.length} ejercicios</b></div><form action={completeWorkout}><input type="hidden" name="assignmentId" value={assignment.id}/>{exercises.map((exercise, index) => <ExerciseExecution exercise={exercise} exerciseIndex={all.indexOf(exercise)} number={index + 1} total={exercises.length} key={`${exercise.name}-${index}`}/>)}<label>Comentario para tu entrenador<textarea name="feedback" placeholder="Sensaciones, molestias o avances…"/></label><button className="button finish-workout" type="submit">Completar y guardar sesión</button></form></section>; }) : <div className="empty"><b>Aún no tienes una rutina asignada</b><p>Tu entrenador debe crear el primer programa.</p></div>}</main>;
}
