...
export default async function NotificationsPage() {
  ...
  return <main className="dashboard narrow"><div className="page-title"><div><span className="kicker">ACTIVIDAD</span><h1>Notificaciones</h1></div>{items.some((item) => !item.readAt) && <form action={markNotificationsRead}><button className="mini-button">Marcar leídas</button></form>}</div><section className="panel"><div className="notification-list">{items.length ? items.map((item) => <a className={item.readAt ? "" : "unread"} href={item.href || "/dashboard"} key={item.id}><span className="dot"/><span><b>{item.title}</b><small>{item.body}</small><time>{item.createdAt.toLocaleString("es")}</time></span></a>) : <div className="empty">No tienes notificaciones.</div>}</div></section></main>;
}
